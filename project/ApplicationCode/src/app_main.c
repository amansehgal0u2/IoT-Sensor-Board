...
//------------------------------------------
// filter defines---------------------------
#define LP_FILT_WIN_SIZE			10
uint32_t t_filt[LP_FILT_WIN_SIZE] = {0};
uint32_t h_filt[LP_FILT_WIN_SIZE] = {0};
uint8_t i = 0,h=0;
uint32_t avg[2]={0};
//------------------------------------------
...
static void mqtt_callback(struct mqtt_module *module_inst, int type, union mqtt_data *data)
{
	...
	case MQTT_CALLBACK_CONNECTED:
		if (data->connected.result == MQTT_CONN_RESULT_ACCEPT) {
			...
			status = mqtt_subscribe(module_inst, PINGSEND_TOPIC,2);
			status = mqtt_subscribe(module_inst, PINGRESP_TOPIC,2);
			status = mqtt_subscribe(module_inst, H_SENSOR_TOPIC,2);
			status = mqtt_subscribe(module_inst, SENSOR_TOPIC,2);
		    status = mqtt_subscribe(module_inst, GET_SENSOR_TOPIC,2);
			status = mqtt_subscribe(module_inst, SENSOR_HEALTH,2);
	...
	case MQTT_CALLBACK_RECV_PUBLISH:
      ...
      if (!strncmp(data->recv_publish.topic, ACTUATOR_TOPIC, strlen(ACTUATOR_TOPIC)) ) 
	  {
		...
		///// ping receive topic
		if (!strncmp(data->recv_publish.topic, PINGSEND_TOPIC, strlen(PINGSEND_TOPIC)) ) 
		{
			if(!strncmp(data->recv_publish.msg,"ping", data->recv_publish.msg_size))
			{
				heartbeat=true;
			}		
		}
		///// sample sensor readings topic
		if (!strncmp(data->recv_publish.topic, GET_SENSOR_TOPIC, strlen(GET_SENSOR_TOPIC)) ) 
		{
			if(!strncmp(data->recv_publish.msg,"sample", data->recv_publish.msg_size))
			{
				sample=true;
			}		
		}
...
void read_sensors()
{
	...
	while(i2c_master_get_job_status	(&i2c_master_instance)); // wait for writes to finish
	uint8_t wr_buffer2[1] = {0x00};
	wr_packet.data = wr_buffer2; //trigger a measurement
	wr_packet.data_length = 1;
	while(i2c_master_write_packet_job(&i2c_master_instance, &wr_packet));// wait for writes to finish
		 
	delay_ms(15); // wait for conversion
		 
	rd_packet.data_length = 4;
	i2c_master_read_packet_job(&i2c_master_instance, &rd_packet);
	t_filt[i++] =  ((uint32_t)rd_packet.data[0]<<8 | (uint32_t)rd_packet.data[1]);
	h_filt[h++] =  ((uint32_t)rd_packet.data[2]<<8 | (uint32_t)rd_packet.data[3]);
	if (i>9) // temperature filter window is primed
	{
		for(int j=0;j<9;j++)
		{
			t_filt[j] = t_filt[j+1];
	...
	if (h>9) // humidity filter window is primed
	{
		for(int j=0;j<9;j++)
		{
			h_filt[j] = h_filt[j+1];
	...
	avg[0]=0;
	for(int j=0;j<10;j++)//10 point average for temp
	{
		avg[0] += t_filt[j];
	}	 
	avg[0]=avg[0]/10; //drop decimal point
	
	avg[1]=0;
	for(int j=0;j<10;j++)//10 point average for humidity
	{
		avg[1] += h_filt[j];
	}
	avg[1]=avg[1]/10; //drop decimal point
	if(*(uint32_t*)rd_packet.data != 0xFFFFFFFF)
	{
		sensor_watchdog++;
	}
}

//---------------------I2C sensor stuff ends-----------------------------------
int main (void)
{
	...
	while (1) {
		...
		{
			read_sensors();
			int32_t t1 = (avg[0]*160)/65535 -40;
			sprintf(pub_text, "%d",t1);
			mqtt_publish(&mqtt_inst, SENSOR_TOPIC, pub_text, strlen(pub_text), 2, 0);
			uint32_t t2 = (avg[1]*100)/65535 ;
			sprintf(pub_text, "%d", t2);
			mqtt_publish(&mqtt_inst, H_SENSOR_TOPIC, pub_text, strlen(pub_text), 2, 0);
			delay = 600000;
		}
		...
		if(heartbeat)
		{
			sprintf(pub_text, "%s", "OK");
			mqtt_publish(&mqtt_inst, PINGRESP_TOPIC, pub_text, strlen("OK"), 2, 0);
			heartbeat=false;
		}
		// sensor health response
		if (sample==true)
		{
			if(sensor_watchdog > 0)
			...
			{
				sprintf(pub_text, "%s", "KO");
			}
			
			mqtt_publish(&mqtt_inst, SENSOR_HEALTH, pub_text, strlen("OK"), 2, 0);
			sample = false;
			sensor_watchdog--;
		}
